...
## 1. 架构定位与设计思想
OpenGPGPU MVP ISA 是一套面向现代 AI 算力需求定义的 128-bit 定长异步流指令集。其核心设计思想为**“计算与访存物理级解耦”**与**“算子宏指令化”**。通过将调度智能上移至编译器，微架构得以大幅简化指令发射（Issue）与计分板（Scoreboard）的硬件复杂度，从而将晶体管资源集中于张量计算阵列。
